...
# This script installs applications specific to CentOS/RHEL
# It installs packages using the `dnf` package manager
# The script assumes that the system is running CentOS or RHEL

# Function to install basic applications for CentOS/RHEL (from the config.yaml list)
install_basic_apps() {
    echo "Installing basic applications on CentOS/RHEL..."
    for app in "${selected_apps[@]}"; do
        case "$app" in
            ...
            "Git")
                ...
                ;;
            *)
                echo "Application $app not found in CentOS/RHEL package repositories. Skipping."
                ;;
        esac
    done
    echo "All selected applications installed successfully on CentOS/RHEL."
}

# Call the installation function for CentOS/RHEL
install_basic_apps
